 ...
 * - 정식 트리거 전환은 자동화 관리 메뉴의 AUTOMATION_executeCanonicalCutover()에서
 *   중앙 디스패처와 5분 핵심 동기화 파이프라인을 함께 설치한다.
 *
 * 주의:
 ...
 * 5분 핵심 동기화 파이프라인이 이 플래그를 안전하게 소비한다.
 */
...
function AUTOMATION_recordCoreFullSyncRequest_(changeType, e) {
  // 모든 설치형 트리거가 bang@s1samsung.com 하나로 실행되므로
  // UserLock으로 구조 변경 요청 기록과 파이프라인의 플래그 삭제를 원자화한다.
  var lock = LockService.getUserLock();

  if (!lock.tryLock(5000)) {
    throw new Error('전체보정 요청 기록용 잠금을 얻지 못했습니다.');
  }
...
/**
 * 핵심 동기화 파이프라인에서 사용할 전체보정 요청 조회 함수.
 */
 ...
 * 파이프라인의 세 단계가 모두 성공한 경우에만 안전 비교 후 삭제한다.
 */
...
function AUTOMATION_runModuleSafely_(summary, moduleName, handlerName, e) {
  ...
  try {
    ...
    var resultStatus = String(result && result.status || 'SUCCESS').toUpperCase();
    row.result = AUTOMATION_makeRuntimeJsonSafe_(result);
    row.retryQueued = !!(result && result.retryQueued);

    if (resultStatus === 'DEFERRED' || resultStatus === 'LEASE_BUSY') {
      row.status = 'DEFERRED';
      row.error = String(result && (result.error || result.reason) || '재처리 큐로 이관됨');

      if (!row.retryQueued && !(e && e.__automationRetryExecution === true)) {
        try {
          var deferredQueueResult = AUTOMATION_enqueueEditRetry_(
            moduleName,
            handlerName,
            e,
            resultStatus,
          ...
          row.retryQueued = !!(deferredQueueResult && deferredQueueResult.queued);
        } catch (deferredQueueErr) {
          row.queueError = AUTOMATION_errorMessage_(deferredQueueErr);
        }
      }
    ...
    if (resultStatus === 'ERROR') {
      row.status = 'ERROR';
      row.error = String(result && (result.error || result.reason) || '모듈 처리 오류');

      if (!row.retryQueued && !(e && e.__automationRetryExecution === true)) {
        try {
          var errorQueueResult = AUTOMATION_enqueueEditRetry_(
            moduleName,
            handlerName,
            e,
            'MODULE_ERROR',
          ...
          row.retryQueued = !!(errorQueueResult && errorQueueResult.queued);
        } catch (errorQueueErr) {
          row.queueError = AUTOMATION_errorMessage_(errorQueueErr);
        }
      }
    ...
    if (resultStatus.indexOf('IGNORED') === 0) {
      row.status = 'IGNORED';
    ...
    if (!(e && e.__automationRetryExecution === true)) {
      try {
        var queueResult = AUTOMATION_enqueueEditRetry_(
          moduleName,
          handlerName,
          e,
        ...
        row.retryQueued = !!(queueResult && queueResult.queued);
      } catch (queueErr) {
        row.queueError = AUTOMATION_errorMessage_(queueErr);
      }
    }
